const ProductsPage = {
  async render() {
    ...
    try {
      ...
      const draw = list => { document.getElementById('productsTable').innerHTML = list.length ? list.map(p => `<tr><td>${p.name}</td><td>${p.sku}</td><td>${p.store || '-'}</td><td>R$ ${Number(p.current_price).toFixed(2)}</td><td><button class="btn btn--small edit-product" data-id="${p.id}">Editar</button> <button class="btn btn--small delete-product" data-id="${p.id}">Excluir</button></td></tr>`).join('') : '<tr><td colspan="5">Nenhum produto encontrado.</td></tr>'; };
      draw(items);
