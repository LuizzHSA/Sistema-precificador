const ProductsPage = {
  async render() {
    ...
    try {
      ...
      const draw = list => {
        document.getElementById('productsTable').innerHTML = list.length ? list.map(p => `<tr><td>${p.name}</td><td>${p.sku}</td><td>${p.store || '-'}</td><td>R$ ${Number(p.current_price).toFixed(2)}</td><td><button class="btn btn--small details-product" data-id="${p.id}">Detalhes</button> <button class="btn btn--small edit-product" data-id="${p.id}">Editar</button> <button class="btn btn--small delete-product" data-id="${p.id}">Excluir</button></td></tr>`).join('') : '<tr><td colspan="5">Nenhum produto encontrado.</td></tr>';
        content.querySelectorAll('.details-product').forEach(btn => btn.onclick = () => this.details(btn.dataset.id));
        content.querySelectorAll('.edit-product').forEach(btn => btn.onclick = () => this.form(items.find(p => p.id === btn.dataset.id), stores));
        content.querySelectorAll('.delete-product').forEach(btn => btn.onclick = () => this.delete(btn.dataset.id));
      };
      draw(items);
      document.getElementById('productSearch').addEventListener('input', async e => {
        try { draw((await api.getProducts({search: e.target.value})).data || []); }
        catch (error) { showToast(error.message, 'error'); }
      });
      document.getElementById('newProduct').onclick = () => this.form(null, stores);
    } catch (error) { content.innerHTML = `<div class="card"><div class="card-body">${error.message}</div></div>`; }
  },
  async delete(productId) {
    if (!confirm('Excluir este produto? O histórico também será removido.')) return;
    try {
      await api.deleteProduct(productId);
      showToast('Produto excluído com sucesso', 'success');
      await this.render();
    } catch (error) { showToast(error.message || 'Não foi possível excluir o produto', 'error'); }
  },
  async details(productId) {
    try {
      const result = await api.getProductHistory(productId);
      const changes = result.data || [], product = result.product, box = document.getElementById('productForm');
